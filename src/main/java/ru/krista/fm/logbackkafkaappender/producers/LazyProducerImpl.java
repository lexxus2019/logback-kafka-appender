...
import ru.krista.fm.logbackkafkaappender.delivery.AsynchronousDeliveryStrategy;
import ru.krista.fm.logbackkafkaappender.delivery.DeliveryStrategy;
...
public class LazyProducerImpl<E> implements KafkaInternalProducer<E> {
    ...
    protected Producer<byte[], byte[]> initialize() {
        ...
        try {
            producer = new org.apache.kafka.clients.producer.KafkaProducer<>(new HashMap<>(producerConfig));
        } catch (Exception e) {
